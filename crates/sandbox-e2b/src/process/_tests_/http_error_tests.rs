//! Envd Reqwest error-classification regressions.
...
#[tokio::test]
async fn connection_failures_are_retryable_for_reads_and_ambiguous_for_mutations() {
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind refused endpoint");
    let address = listener.local_addr().expect("refused endpoint address");
    drop(listener);
    let url = format!("http://{address}/unavailable");
    let read_error = reqwest::Client::new()
        .get(&url)
        .send()
        .await
        .expect_err("closed endpoint should refuse the read");
    let mutation_error = reqwest::Client::new()
        .post(url)
        .send()
        .await
        .expect_err("closed endpoint should refuse the mutation");

    assert!(matches!(
        request_error(read_error, false, "test read"),
        E2bAdapterError::Unavailable
    ));
    assert!(matches!(
        request_error(mutation_error, true, "test mutation"),
        E2bAdapterError::DeliveryAmbiguous
    ));
}

#[tokio::test]
async fn truncated_response_bodies_are_retryable_transport_failures() {
