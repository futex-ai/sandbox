...
- The default `cargo xtask check` intentionally omits the heavyweight
  `fna-cli` phase. When changes affect `fna-cli`, its server/worker runtime
  boundary, or app packaging and submission through the CLI, also run
  `cargo xtask check --include fna-cli`; pull-request CI runs that phase in its
  dedicated job for every change.
...
- Apps platform logic in this repository must remain app-agnostic. It may contain
  only generic, reusable capabilities that work across all apps. App-specific
  behavior, data, configuration, identifiers, workflows, and exceptions belong
  entirely in `firna-apps` and must never be implemented in this repository.
...
- When implementing UI under `ts/app`, first check `@firna/ui` and use its
  components whenever it provides a relevant component for the task or view.
  When `@firna/ui` has no relevant component, check the available reusable/base
  components in `ts/app/src/ui` before creating a new one. If a component or
  pattern is used many times across the app, extract it or add it to
  `ts/app/src/ui` instead of duplicating the implementation.
- Never use left-border emphasis anywhere in the product or its designs: no
  colored accent stripe along one edge of a card, banner, callout, toast,
  quote, or selected state, in `ts/app`, `ts/web`, `docs/mockups`, or any
  other user-facing surface. Convey tone with a full-perimeter border, a
  tinted fill, or a leading icon instead. Neutral structural borders such as
  column dividers and nesting indent guides are layout, not emphasis, and
  remain fine.
...
- Milestones that include UI or mockup work must include an explicit tag line
  ...
  work is found to be missing while implementing a `ui` milestone, always
  create new milestones; never add the backend work to the current milestone
  or an existing milestone, and never move the blocked UI TODOs into an
  existing milestone. First create a new backend milestone immediately after
  the current milestone for the required backend work. Then create a new tagged
  UI milestone immediately after that backend milestone and move the blocked
  UI TODOs there, preserving their incomplete status. If backend work is found
  while implementing the mockup milestone, add a new backend milestone after
  the mockup milestone without splitting or moving it; backend discovery alone
  is not a reason to add another mockup milestone.
...
- Any time a new TODO required to complete the PR is discovered during
  implementation, it should be added under the relevant milestone (just add
  the new TODO, and then continue with the active TODO). Record merge-dependent
  follow-up work in the non-blocking post-merge section instead.
